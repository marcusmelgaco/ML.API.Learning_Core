...
mlp = MLPRegressor(hidden_layer_sizes=(6,6,6), activation='relu', verbose=True, max_iter=1500, solver='lbfgs', random_state = 12)
...
class MLPRegressorAlgorithm:
    ...
    def learning(self, x_training, y_training):
        ...
        mlp.fit(self.x_training_scaler,self.y_training_scaler.ravel());
        self.determination_coefficient_training = mlp.score(self.x_training_scaler, self.y_training_scaler);
    ...
    def predict(self, x_test, y_test=[]):
        ...
        if(len(y_test)):
            self.determination_coefficient_test = mlp.score(self.x_test_scaler, self.y_test_scaler);

        self.predict_test = mlp.predict(self.x_test_scaler);
    ...
    def validateAlgorithm(self, forecasters_base, target_base):
        ...
        self.average_cross_validation = cross_val_score(mlp, self.forecasters_base_scaler, self.target_base_scaler.ravel(), cv = kfold).mean() * 100.0;
