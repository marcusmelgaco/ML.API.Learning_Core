...
class DecisionTreeAlgorithm:
    ...
    def learning(self, x_training, y_training):
        ...
        self.determination_coefficient_training = tree.score(self.x_training, self.y_training);
        
    def predict(self, x_test, y_test=[]):
        self.x_test = x_test;
        self.y_test = y_test;
        self.determination_coefficient_test = tree.score(self.x_test, self.y_test);

        self.predict_test = tree.predict(x_test);
    ...
    def validateAlgorithm(self, forecasters_base, target_base):
        ...
        self.predict_training = tree.predict(self.x_training);
        ...
        self.average_cross_validation = cross_val_score(tree, forecasters_base, target_base, cv = kfold).mean() * 100.0;
