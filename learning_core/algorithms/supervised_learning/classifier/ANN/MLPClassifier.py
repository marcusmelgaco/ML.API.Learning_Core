...
mlp = MLPClassifier(hidden_layer_sizes=(7), activation='relu', solver='adam', max_iter =800, tol=0.0001, random_state = 3, verbose = True)
kfold = KFold(n_splits = 30, shuffle=True, random_state = 5)


class MLPClassifierAlgorithm():
    forecasters_base = [];
    target_base = [];
    ...
    def predict(self, x_test, y_test=[], accuracy=True):
        ...
        self.predict_test = mlp.predict(x_test);
    ...
    def validateAlgorithm(self, forecasters_base, target_base):
        ...
        self.predict_training = mlp.predict(self.x_training);
        ...
        self.average_cross_validation = cross_val_score(mlp, forecasters_base, target_base, cv = kfold).mean() * 100.0;
